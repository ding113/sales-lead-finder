...
export const SummaryModal: React.FC<Props> = ({
  ...
  useEffect(() => {
    if (isOpen && distributors.length > 0) {
      ...
      generateSummary(distributors).then((summaryResult) => {
        const newMessages: Message[] = [
          {
            type: "assistant",
            content:
              "👋 Hi! I've analyzed your search results. Here's what I found:",
          },
          {
            type: "data",
            content: (
              <div className="space-y-2">
                ...
                <ul className="list-disc list-inside space-y-1 text-gray-600">
                  ...
                  <li>
                    {Object.keys(summaryResult.industryDistribution).length}{" "}
            ...
            content: (
              <div className="space-y-3">
                {summaryResult.topDistributors.slice(0, 3).map((dist, idx) => (
                  <div
                    key={dist.id}
                    className="flex items-center space-x-3 p-2 rounded-lg bg-indigo-50/50"
                  >
                    <span className="text-lg">{["🥇", "🥈", "🥉"][idx]}</span>
                    <div>
                      <p className="font-medium">{dist.companyName}</p>
                      <p className="text-sm text-gray-600">
                        {dist.industry.join(", ")} • {dist.rating}⭐
                      </p>
                    </div>
                  </div>
                ))}
