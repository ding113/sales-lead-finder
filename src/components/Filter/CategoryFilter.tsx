...
const CategoryFilter: React.FC<CategoryFilterProps> = ({
  ...
  const handleSelectAllInCategory = (category: Category) => {
    const subCategoryValues = category.subCategories?.map(sub => sub.value) || [];
    const allSelected = subCategoryValues.every(value => selectedValues.includes(value));

    if (allSelected) {
      onChange(selectedValues.filter(value => !subCategoryValues.includes(value)));
    } else {
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2 max-h-80 overflow-y-auto">
        {filteredCategories.map(category => (
          <div key={category.value} className="border border-gray-200 rounded-md">
            <button
            ...
            >
              ...
              <ChevronDownIcon
                className={`h-5 w-5 text-gray-400 transform transition-transform ${
                  expandedCategories.includes(category.value) ? 'rotate-180' : ''
                }`}
              />
            </button>

            <AnimatePresence>
              {expandedCategories.includes(category.value) && category.subCategories && (
                <motion.div
                  initial={{ height: 0 }}
                  animate={{ height: 'auto' }}
                  exit={{ height: 0 }}
                ...
                >
                  ...
                  <div className="px-4 py-2 space-y-2">
                    {category.subCategories.map(subCategory => (
                      <label key={subCategory.value} className="flex items-center text-sm">
                        <input
                          type="checkbox"
                          className="rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                          checked={selectedValues.includes(subCategory.value)}
                          onChange={() => handleSelectSubCategory(subCategory.value)}
                        />
                        <span className="ml-2 text-gray-600">{subCategory.label}</span>
                      </label>
                    ))}
                  </div>
                </motion.div>
